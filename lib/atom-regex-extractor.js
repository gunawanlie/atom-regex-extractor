...
export default {
  ...
  extract() {
    let uri = atom.workspace.activePaneContainer.paneContainer.activePane.activeItem.uri;

    if (typeof uri === "string" && uri === "atom://find-and-replace/project-results") {
      results = this.extractFindProjectResults();
      this.openNewFile(results);
    } else {
      results = this.extractFindResults();
      this.openNewFile(results);
    }
  },
